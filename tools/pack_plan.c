...
static void measure(unsigned capacity, unsigned frames) {
  ...
  values[FG_CONFIG_MAX_GRAINS] = capacity;
  ...
  REQUIRE(engine);
  for (unsigned i = 0; i < 997; ++i) fg_source(engine)[i] = .3*sin((double)i);
  bytes = fg_grain_plan_size(&config, frames);
  FGGrainPlan *plan = fg_grain_plan_init(malloc(bytes), bytes, &config, frames);
  size_t packet_bytes = fg_pack_size_frames(capacity, frames);
  size_t scratch_bytes = fg_pack_scratch_size_frames(capacity, frames);
  void *packet = malloc(packet_bytes), *scratch = malloc(scratch_bytes);
  REQUIRE(plan && packet && scratch);
  controls[FG_CONTROL_GRAIN_RATE] = capacity == 128 ? 800 : 2000;
  controls[FG_CONTROL_GRAIN_MS] = 500;
  controls[FG_CONTROL_GAIN] = .15;
   ...
   * deliberately not measured. All work outside fg_pack is untimed for it. */
  for (unsigned first = 0; first < 49152; first += frames) {
    double start = now();
    REQUIRE(fg_grain_plan_begin(engine, plan));
    for (unsigned f = 0; f < frames; ++f) REQUIRE(fg_grain_plan_sample(engine, plan));
    ...
    records += view.grains;
    start = now();
    size_t used = fg_pack(plan, packet, packet_bytes, scratch, scratch_bytes);
    packing += now() - start;
    REQUIRE(used);
    transferred += used;
    ...
    double output[2*FG_GRAIN_PLAN_MAX_FRAMES] = {0};
    REQUIRE(fg_grain_plan_commit(engine, plan, output, 2*frames));
  }
