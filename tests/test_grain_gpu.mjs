...
const {values}=parseArgs({options:{playwright:{type:'string'},browser:{type:'string'},url:{type:'string',default:'http://127.0.0.1:8784'},output:{type:'string',default:'build/compact-gpu'},software:{type:'boolean',default:false}}});
...
try {
 ...
 receipt.result=await page.evaluate(()=>new Promise((resolve,reject)=>{
  const worker=new Worker('./test_grain_gpu.worker.js',{type:'module'});
  const timer=setTimeout(()=>{worker.terminate();reject(new Error('Real-grain GPU test timed out'));},300000);
 ...
 await pending;assert.equal(receipt.rows.length,9);receipt.status='complete';await save();
