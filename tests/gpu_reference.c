...
int main(void) {
  ...
  for(unsigned n=16;n<=64;n*=2) for(unsigned mode=0;mode<11;mode++) {
    ...
    if(mode==2) {
      c[FG_CONTROL_VISCOSITY]=.002;
      for(unsigned j=0;j<n;j++)for(unsigned i=0;i<n;i++)
        f.u[j*n+i]=cos(TAU*(j+.5)/n);
    }
    if(mode>=4 && mode<=6) {
      c[FG_CONTROL_SWIRL]=mode==4?-.8:0;
      c[FG_CONTROL_TURBULENCE]=mode==5?1:0;
      c[FG_CONTROL_STRAIN_DRIVE]=mode==6?-.7:0;
      c[FG_CONTROL_VISCOSITY]=0;
    }
    if(mode==7)c[FG_CONTROL_CONFINEMENT]=.7;
    if(mode==8) {
      c[FG_CONTROL_VISCOSITY]=.01;c[FG_CONTROL_DRIVE]=2;
      c[FG_CONTROL_SWIRL]=-1;c[FG_CONTROL_TURBULENCE]=1;
      c[FG_CONTROL_STRAIN_DRIVE]=-1;c[FG_CONTROL_CONFINEMENT]=1;
      c[FG_CONTROL_FLOW_SPEED]=4;c[FG_CONTROL_EDDY_SIZE]=.05;
    }
    ...
    array(c,FG_CONTROL_COUNT);
    printf(",\"initial\":");array(f.u,2*count);
    for(unsigned step=0;step<steps;step++)
      if(!fg_field_step(&f,c,60,240,pressure,diffusion)){free(storage);return 2;}
