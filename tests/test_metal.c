...
int ng_test_device_render(NGDeviceGPU *g,const void *p,size_t bytes,uint32_t n,double *out) {
  if(reject_next){reject_next=0;for(unsigned i=0;i<2*n;++i)out[i]=NAN;return 0;}
  return ng_device_gpu_render(g,p,bytes,n,out);
}
...
static void run(unsigned frames,int device,int loop,int inject,int carrier) {
  ...
  if(carrier){CHECK(ng_source_cosine(ref));CHECK(ng_source_cosine(actual));}
  if(carrier==2){CHECK(ng_sound_space(ref,55,12000));CHECK(ng_sound_space(actual,55,12000));}
