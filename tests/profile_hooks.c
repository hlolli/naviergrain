...
size_t __real_ng_memory_size(const NGConfig *, size_t, double, double);
...
size_t __wrap_ng_memory_size(const NGConfig *c, size_t length, double source_sr,
                            double engine_sr) {
  arena_bytes = __real_ng_memory_size(c, length, source_sr, engine_sr);
  return arena_bytes;
}
int __wrap_ng_field_step(NGField *f, const double *c, double fluid_hz,
                        double particle_hz, unsigned pressure, unsigned viscosity) {
  if (!active)
    return __real_ng_field_step(f, c, fluid_hz, particle_hz, pressure, viscosity);
  struct timespec begin, end;
  int start_ok = clock_gettime(CLOCK_MONOTONIC, &begin) == 0;
  int result = __real_ng_field_step(f, c, fluid_hz, particle_hz, pressure, viscosity);
...
EXPORT void ng_profile_begin(void) {
  steps = step_ns = maximum_ns = allocations = clock_errors = 0;
  active = 1;
}
EXPORT void ng_profile_end(double values[6]) {
  active = 0;
  values[0] = steps;
  values[1] = step_ns;
  values[2] = maximum_ns;
