...
for(const file of ['src/naviergrain_core.c','src/naviergrain_grain_plan.h','src/naviergrain_pack.c','src/naviergrain_pack.h','src/naviergrain_plan_opcode.inc','web/grain-plan.ts','web/grain-plan.wgsl','web/grain-renderer.ts','tests/test_grain_delivery.worker.ts','build/wasm/naviergrain.wasm'])receipt.hashes[file]=createHash('sha256').update(await readFile(file)).digest('hex');
