...
int __real_fg_cuda_render(FGCuda *,const void *,size_t,uint32_t,double *);
int __wrap_fg_cuda_render(FGCuda *g,const void *p,size_t bytes,uint32_t n,double *out) {
  ...
  return __real_fg_cuda_render(g,p,bytes,n,out);
}
static void controls(unsigned batch,double *c) {
  double config[FG_CONFIG_COUNT];fg_defaults(config,c);
  c[FG_CONTROL_GAIN]=.15;c[FG_CONTROL_GRAIN_RATE]=1000;c[FG_CONTROL_GRAIN_MS]=80;
  c[FG_CONTROL_PITCH_RATIO]=batch<12?.5:2.25;
  c[FG_CONTROL_RESET]=batch==16;c[FG_CONTROL_FREEZE]=batch>=23&&batch<26;
}
static void run(int device,unsigned fault,int disable) {
  ...
  fail_call=fault;
  double values[FG_CONFIG_COUNT],c[FG_CONTROL_COUNT],source[997];fg_defaults(values,c);
  ...
  FGGPUWorker *w=fg_gpu_worker_create(&config,source,997,36000,48000,512,device);CHECK(w);
  ...
  double out[1024],expected[1024];FGGPUResult result;
  memset(out,0x5a,sizeof(out));memset(&result,0x5a,sizeof(result));
  CHECK(fg_gpu_worker_read(w,out,1024,&result)==0);
  CHECK(out[0]!=0&&result.frames==0x5a5a5a5a);
  CHECK(fg_gpu_worker_submit(w,c,0,1)==-1);
  CHECK(fg_gpu_worker_submit(w,c,513,1)==-1);
  for(unsigned b=0;b<4;++b){controls(b,c);CHECK(fg_gpu_worker_submit(w,c,512,1)==1);}
  c[FG_CONTROL_RESET]=1;
  CHECK(fg_gpu_worker_submit(w,c,512,0)==0); /* rejected reset/downgrade */
  if(device==0) {
    ...
    CHECK(fg_gpu_worker_read(w,out,1024,&result)==0);
    CHECK(fg_gpu_worker_submit(w,c,512,0)==0);
    atomic_store(&hold,0);
  }
  ...
  for(unsigned b=0;b<40;++b) {
    ...
    for(unsigned i=0;i<20000;++i) {
      read=fg_gpu_worker_read(w,out,1,&result);
      if(read)break;
      pause_short();
    }
    CHECK(read==-1); /* insufficient output must not consume */
    CHECK(fg_gpu_worker_read(w,out,1024,&result)==1);
    unsigned n=b==39?7:512;
    CHECK(result.ok&&result.sequence==b&&result.first_frame==frame&&result.frames==n);
    frame+=n;controls(b,c);fg_controls(ref,c);
    for(unsigned f=0;f<n;++f)fg_sample(ref,&expected[2*f],&expected[2*f+1]);
    ...
    double stats[FG_STAT_COUNT];fg_stats(ref,stats);
    for(unsigned i=0;i<FG_STAT_COUNT;++i) {
      if(i==FG_STAT_PRE_LIMITER_PEAK)CHECK(fabs(stats[i]-result.stats[i])<1e-5);
      else CHECK(stats[i]==result.stats[i]);
    }
    if(submitted<40) {
      controls(submitted,c);n=submitted==39?7:512;
      CHECK(fg_gpu_worker_submit(w,c,n,!(disable&&submitted==20))==1);
      ++submitted;
    }
  }
  ...
  CHECK(fg_gpu_worker_submit(w,c,512,1)==1);
  fg_gpu_worker_destroy(w);free(ref);
...
static void plugin(const char *path) {
  ...
  const FGNativeGPUWorkerAPI *(*entry)(void);
  *(void **)(&entry)=dlsym(module,"fluidgrain_native_gpu_api");CHECK(entry);
  const FGNativeGPUWorkerAPI *api=entry();CHECK(api&&api->version==1);
  double values[FG_CONFIG_COUNT],c[FG_CONTROL_COUNT],source[997];fg_defaults(values,c);
  values[FG_CONFIG_GRID_SIZE]=16;values[FG_CONFIG_MAX_GRAINS]=128;
  FGConfig config;CHECK(!fg_config_parse(&config,values,FG_CONFIG_COUNT));
  for(unsigned i=0;i<997;++i)source[i]=.2*sin(6.283185307179586*i/37);
  FGGPUWorker *w=api->create(&config,source,997,48000,48000,512,0);CHECK(w);
  size_t bytes=fg_memory_size(&config,997,48000,48000);
  FGEngine *ref=fg_init(malloc(bytes),bytes,&config,997,48000,48000);CHECK(ref);
  memcpy(fg_source(ref),source,sizeof(source));
  double peak=0;
  for(unsigned b=0;b<12;++b) {
    controls(b,c);CHECK(api->submit(w,c,512,b!=6)==1);
    double out[1024],expected[1024];FGGPUResult result;int ready=0;
    ...
    fg_controls(ref,c);
    for(unsigned f=0;f<512;++f)fg_sample(ref,&expected[2*f],&expected[2*f+1]);
