...
const [file='build/ui/naviergrain-live.wasm',rate='32000',duration='80',views='1',stress='0']=process.argv.slice(2);
const schema=JSON.parse(fs.readFileSync(new URL('../schema/naviergrain-v1.json',import.meta.url)));
...
w.ng_live_control(live,1,Number(rate));w.ng_live_control(live,2,Number(duration));w.ng_live_control(live,20,0);
if(Number(stress)){
  for(const i of [3,6,8,9,10,11,12,13,14,17,18,19,20])
    if(!w.ng_live_control(live,i,schema.control[i].max))throw Error('Invalid stress control');
}
let queued=2048,under=0,peak=0,renderMs=0,maxBatch=0;
for(let i=0;i<4;i++)w.ng_live_render(live,512);
let last=performance.now();const start=last;
for(let batch=0;batch<192;batch++){
  if(queued>=2048)await delay((queued-1536)/48);
  const before=performance.now();
  if(!w.ng_live_render(live,512))throw Error('Render failed');
  if(Number(views)&&batch%2===0){w.ng_live_particles(live);w.ng_live_spectrum(live);}
  ...
  peak=Math.max(peak,new Float64Array(w.memory.buffer,w.ng_live_stats(live),20)[liveStat]);
}
