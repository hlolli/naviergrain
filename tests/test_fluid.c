...
static FGField make(unsigned n) {
  FGField f;
  double *storage = calloc(fg_field_doubles(n), sizeof(double));
  CHECK(storage);
  fg_field_init(&f, n, storage, 7);
  return f;
}
static void zero_constant_and_sampling(void) {
  FGField f = make(16);
  double config[FG_CONFIG_COUNT], c[FG_CONTROL_COUNT];
  fg_defaults(config, c);
  c[FG_CONTROL_DRIVE] = 0;
  CHECK(fg_field_step(&f, c, 60, 240, 64, 16));
  ...
  FGFieldSample s = fg_field_sample(&f, -.001, 1.99);
  CHECK(s.u == .25 && s.v == -.125 && s.omega == 0 && s.strain == 0);
  CHECK(f.interventions == 0 && f.rms_divergence == 0);
  c[FG_CONTROL_FREEZE] = 1;
  CHECK(fg_field_step(&f, c, 60, 240, 64, 16) && f.sequence == 2);
  free(f.storage);
}
static void operators_and_projection(void) {
  ...
  fg_field_gradient(&f, f.psi, f.u, f.v);
  fg_field_divergence(&f, f.u, f.v, f.au);
  double eigenvalue =
  ...
  fg_field_project(&f, f.u, f.v, 4096);
  double gauge = 0;
  for (unsigned k = 0; k < 256; ++k) {
...
static void diffusion_and_transport(void) {
  for (unsigned n = 16; n <= 64; n *= 2) {
    ...
    fg_field_diffuse(&f, f.u, 0, 16);
    CHECK(memcmp(f.psi, f.u, (size_t)n * n * sizeof(double)) == 0);
    fg_field_diffuse(&f, f.u, a, 64);
    ...
    double cfg[FG_CONFIG_COUNT], c[FG_CONTROL_COUNT];
    fg_defaults(cfg, c);
    c[FG_CONTROL_VISCOSITY] = c[FG_CONTROL_DRIVE] = 0;
    CHECK(fg_field_step(&f, c, 60, 240, 64, 16));
    double shift = .2 * dt * n;
    for (unsigned j = 0; j < n; ++j) {
...
static void forcing_scaling_and_recovery(void) {
  FGField a = make(16), b = make(16);
  double cfg[FG_CONFIG_COUNT], c[FG_CONTROL_COUNT];
  fg_defaults(cfg, c);
  c[FG_CONTROL_TURBULENCE] = 0;
  c[FG_CONTROL_STRAIN_DRIVE] = 0;
  c[FG_CONTROL_SWIRL] = 1;
  c[FG_CONTROL_VISCOSITY] = 0;
  CHECK(fg_field_step(&a, c, 60, 240, 64, 16));
  c[FG_CONTROL_DRIVE] *= .5;
  CHECK(fg_field_step(&b, c, 30, 240, 64, 16));
  for (unsigned k = 0; k < 256; ++k)
  ...
  c[FG_CONTROL_DRIVE] = NAN;
  CHECK(!fg_field_step(&a, c, 60, 240, 64, 16));
  CHECK(a.sequence == sequence && !a.valid && a.interventions == 1);
  CHECK(memcmp(saved, a.u, sizeof(saved)) == 0);
  c[FG_CONTROL_DRIVE] = .5;
  CHECK(fg_field_step(&a, c, 60, 240, 64, 16) && a.valid);
  /* Force/speed extrema must stay finite, with an honest intervention. */
  c[FG_CONTROL_DRIVE] = 2;
  c[FG_CONTROL_SWIRL] = 1;
  c[FG_CONTROL_EDDY_SIZE] = .05;
  c[FG_CONTROL_FLOW_SPEED] = 4;
  c[FG_CONTROL_TURBULENCE] = 1;
  c[FG_CONTROL_CONFINEMENT] = 1;
  for (unsigned i = 0; i < 180; ++i)
    CHECK(fg_field_step(&a, c, 30, 60, 16, 4));
...
static double rk_error(unsigned steps) {
  ...
  f.max_speed = .3;
  FGParticle p = {.x = .6, .y = .6, .path_x = .6, .path_y = .6};
  for (unsigned i = 0; i < steps; ++i)
    CHECK(!fg_particle_advance(&f, &p, .1 / steps, 0, 0));
...
static void profiles(void) {
  for (unsigned profile = 0; profile < 4; ++profile) {
    ...
    for (unsigned tick = 0; tick < 300; ++tick) {
      CHECK(fg_field_step(&f, c, 60, 240, passes, 16));
