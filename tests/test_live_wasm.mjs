...
const temporary=mkdtempSync(join(tmpdir(),'naviergrain-live-'));
try {
  ...
  const live=w.ng_live_create(48000,-1);assert(live);let offset=0,error=0;const history=[];
  ...
  for(let b=0;b<32;++b){
    assert(w.ng_live_control(live,3,55));assert(w.ng_live_control(live,10,b<12?.7:-.7));
    assert(w.ng_live_control(live,21,Number(b>=16&&b<20)));if(b===24)assert(w.ng_live_control(live,22,1));
    assert(w.ng_live_render(live,512));compare(new Float64Array(w.memory.buffer,w.ng_live_audio(live),1024),1e-9);
    const audio=new Float64Array(w.memory.buffer,w.ng_live_audio(live),1024);
    for(let f=0;f<512;++f)history.push((audio[f*2]+audio[f*2+1])*.5);
  }
  compare(new Float64Array(w.memory.buffer,w.ng_live_view(live),3344),1e-8);
  ...
  compare(new Float64Array(w.memory.buffer,w.ng_live_spectrum(live),96),1e-5);
  // Independent scalar DFT checks the FFT's window, scale, ordering and bands.
  const spectrum=new Float64Array(w.memory.buffer,w.ng_live_spectrum(live),96),tail=history.slice(-2048);
  ...
  assert.equal(offset,native.byteLength);w.ng_live_destroy(live);
