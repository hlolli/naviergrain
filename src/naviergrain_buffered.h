#ifndef NAVIERGRAIN_BUFFERED_H
#define NAVIERGRAIN_BUFFERED_H
#include "naviergrain_core.h"
typedef struct NGBuffered NGBuffered;
/* Host preparation API exported as naviergrain_native_buffered_api().
 ...
 *
 * aL,aR,kStats[],kGPU,kUnderruns,kQueued,kPlayed naviergrain_buffered
...
typedef struct {
  unsigned version;
  NGBuffered *(*create)(const NGConfig *, const double *, size_t, double,
                       double, uint32_t, int);
  int (*destroy)(NGBuffered *);
  int (*register_buffer)(void *csound, unsigned key, NGBuffered *);
  int (*unregister_buffer)(void *csound, NGBuffered *);
} NGNativeBufferedAPI;
#endif
