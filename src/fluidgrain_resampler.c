...
static double band_ratio(double log_range, unsigned band) {
  ...
  return fmin(DBL_MAX, exp(log_range * (fmin(band + 1.0, FG_SINC_BANDS - 1.0) /
                                        (FG_SINC_BANDS - 1.0))));
}
static unsigned radius_for(double ratio) {
  return (unsigned)fmin(FG_SINC_MAX_RADIUS, ceil(32 * ratio));
}
size_t fg_resampler_doubles(double max_increment) {
  ...
  for (unsigned b = 0; b < FG_SINC_BANDS; ++b)
    count += 2u * radius_for(band_ratio(range, b)) * (FG_SINC_PHASES + 1u);
  return count;
}
void fg_resampler_init(FGResampler *r, double *storage, double max_increment) {
  r->cosine = 0;
  ...
  for (unsigned b = 0; b < FG_SINC_BANDS; ++b) {
    ...
    for (unsigned p = 0; p <= FG_SINC_PHASES; ++p) {
      double sum = 0, phase = (double)p / FG_SINC_PHASES;
      for (unsigned t = 0; t < taps; ++t) {
        double x = (double)t - radius + 1 - phase;
        double angle = 2 * FG_PI * cutoff * x;
        double sinc = fabs(angle) < 1e-12 ? 1 : sin(angle) / angle;
        double window = .42 + .5 * cos(FG_PI * x / radius) +
                        .08 * cos(2 * FG_PI * x / radius);
...
static double convolve_pair(const FGResampler *r, unsigned band, const double *s,
...
static double convolve(const FGResampler *r, unsigned band, const double *s,
...
double fg_read_bandlimited(const FGResampler *r, const double *s, size_t n,
  ...
  if (loop) {
    ...
    return 0;
  if (r->cosine)
    return increment < .5 * (double)n ? fg_osc_cos(2 * FG_PI * phase / (double)n) : 0;
  ...
  if (blend > 0 && band + 1u < FG_SINC_BANDS) {
...
double fg_read_preview(const double *s, size_t n, double phase, int loop,
                       double edge) {
  if (!n || !isfinite(phase))
    return 0;
  if (loop) {
