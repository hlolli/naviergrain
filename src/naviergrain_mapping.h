#ifndef NAVIERGRAIN_MAPPING_H
#define NAVIERGRAIN_MAPPING_H
#include <math.h>
#define NG_SOUND_HEIGHT 3.6
...
static inline void ng_sound_position(double x,double y,double speed,double out[3]) {
  ...
  out[0]=cos(angle)*radius;out[1]=(y-.5)*NG_SOUND_HEIGHT;out[2]=sin(angle)*radius;
}
...
static inline double ng_cosine_bend(double height, double speed) {
  double activity=speed/(1+speed);
  return .75*(2*height-1)+.25*(2*activity-1);
}
static inline double ng_cosine_duration(double milliseconds,double mix,
