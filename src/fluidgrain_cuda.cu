...
extern "C" void fg_cuda_destroy(FGCuda *g) {
  if(!g)return;
...
extern "C" FGCuda *fg_cuda_create(const void *resources,size_t bytes,
  ...
  if(header[0]!=0x52474746u||(header[1]!=1&&header[1]!=2)||!header[2])return nullptr;
  if(cudaSetDevice(device)!=cudaSuccess)return nullptr;
  auto *g=new(std::nothrow) FGCuda;
  if(!g)return nullptr;
  ...
  if(!g->host||
    ...
    fg_cuda_destroy(g);return nullptr;
  }
  return g;
}
extern "C" int fg_cuda_render(FGCuda *g,const void *packet,size_t bytes,
  ...
  /* Packet is owner-generated by fg_pack; overlapping segments are impossible.
