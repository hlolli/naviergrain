...
#include <cstring>
#include <new>
#include "naviergrain_metal_source.h"
...
extern "C" void ng_device_gpu_destroy(NGDeviceGPU *g) { delete g; }
extern "C" NGDeviceGPU *ng_device_gpu_create(const void *resources,size_t bytes,
  ...
  @autoreleasepool {
    ...
    if(h[0]!=0x5247474eu||(h[1]!=1&&h[1]!=2)||!h[2])return nullptr;
    NSArray<id<MTLDevice>> *devices=MTLCopyAllDevices();
    if((NSUInteger)device>=devices.count)return nullptr;
    auto *g=new(std::nothrow) NGDeviceGPU{};
    if(!g)return nullptr;
    ...
    NSError *error=nil;
    id<MTLLibrary> library=[g->device newLibraryWithSource:@(ng_metal_source) options:options error:&error];
...
extern "C" int ng_device_gpu_render(NGDeviceGPU *g,const void *packet,size_t bytes,
