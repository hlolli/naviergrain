...
size_t fg_packet_size(unsigned n) {
  return n == 16 || n == 32 || n == 64 ? FG_PACKET_HEADER + 16u * (size_t)n * n : 0;
}
static int field_storage(const FGField *f) {
  return f && fg_packet_size(f->n) && f->u && f->v && f->omega && f->strain;
}
FGPacketResult fg_packet_encode(void *packet, size_t bytes, const FGField *f,
                                uint64_t instance_id, uint64_t epoch,
                                uint64_t audio_frame) {
  if (!packet || !field_storage(f) || bytes != fg_packet_size(f->n) ||
      !instance_id || !epoch || !f->sequence || !f->valid)
    return FG_PACKET_MALFORMED;
  if (!isfinite(f->time)) return FG_PACKET_NONFINITE;
  if (f->time < 0) return FG_PACKET_MALFORMED;
  ...
  for (unsigned i = 0; i < 6; ++i) {
    if (!isfinite(diagnostics[i]) || diagnostics[i] > FLT_MAX)
      return FG_PACKET_NONFINITE;
    if (diagnostics[i] < 0) return FG_PACKET_MALFORMED;
  }
  ...
  for (unsigned a = 0; a < 4; ++a)
    for (size_t i = 0; i < count; ++i) {
      double v = arrays[a][i];
      if (!isfinite(v) || fabs(v) > FLT_MAX) return FG_PACKET_NONFINITE;
      if (a == 3 && v < 0) return FG_PACKET_MALFORMED;
    }
  unsigned char *p = packet;
  memset(p, 0, FG_PACKET_HEADER);
  memcpy(p, "FGFL", 4);
  write32(p + 4, FG_PACKET_VERSION);
  write32(p + 8, FG_PACKET_HEADER);
  write32(p + 12, (uint32_t)(bytes - FG_PACKET_HEADER));
  ...
  for (unsigned a = 0; a < 4; ++a)
    for (size_t i = 0; i < count; ++i)
      write_float(p + FG_PACKET_HEADER + 4 * (a * count + i), arrays[a][i]);
  return FG_PACKET_OK;
}
FGPacketResult fg_packet_inspect(const void *packet, size_t bytes, FGPacketInfo *info) {
  if (!packet || !info || bytes < FG_PACKET_HEADER || bytes > FG_PACKET_MAX_BYTES)
    return FG_PACKET_MALFORMED;
  const unsigned char *p = packet;
  if (memcmp(p, "FGFL", 4) || read32(p + 8) != FG_PACKET_HEADER ||
      read32(p + 12) != bytes - FG_PACKET_HEADER)
    return FG_PACKET_MALFORMED;
  if (read32(p + 4) != FG_PACKET_VERSION || read32(p + 64) != 1 ||
      read32(p + 68) != 1) return FG_PACKET_INCOMPATIBLE;
  unsigned n = read32(p + 56);
  if (!fg_packet_size(n) || read32(p + 60) != n) return FG_PACKET_INCOMPATIBLE;
  if (bytes != fg_packet_size(n)) return FG_PACKET_MALFORMED;
  for (unsigned i = 104; i < FG_PACKET_HEADER; ++i)
    if (p[i]) return FG_PACKET_MALFORMED;
  FGPacketInfo m = {
  ...
  if (!m.instance_id || !m.epoch || !m.sequence) return FG_PACKET_MALFORMED;
  if (!isfinite(m.simulation_time)) return FG_PACKET_NONFINITE;
  if (m.simulation_time < 0) return FG_PACKET_MALFORMED;
  for (unsigned i = 72; i < 96; i += 4) {
    double v = read_float(p + i);
    if (!isfinite(v)) return FG_PACKET_NONFINITE;
    if (v < 0) return FG_PACKET_MALFORMED;
  }
  size_t strain_offset = FG_PACKET_HEADER + 12u * (size_t)n * n;
  for (size_t i = FG_PACKET_HEADER; i < bytes; i += 4) {
    double v = read_float(p + i);
    if (!isfinite(v)) return FG_PACKET_NONFINITE;
    if (i >= strain_offset && v < 0) return FG_PACKET_MALFORMED;
  }
  *info = m;
  return FG_PACKET_OK;
}
static void apply_packet(const unsigned char *p, const FGPacketInfo *m, FGField *f) {
  double *arrays[] = { f->u, f->v, f->omega, f->strain };
  size_t count = (size_t)m->n * m->n;
  for (unsigned a = 0; a < 4; ++a)
    for (size_t i = 0; i < count; ++i)
      arrays[a][i] = read_float(p + FG_PACKET_HEADER + 4 * (a * count + i));
...
FGPacketResult fg_packet_decode(const void *packet, size_t bytes, FGField *f,
                                FGPacketInfo *info) {
  if (!field_storage(f) || !info) return FG_PACKET_MALFORMED;
  FGPacketInfo m;
  FGPacketResult result = fg_packet_inspect(packet, bytes, &m);
  if (result != FG_PACKET_OK) return result;
  if (m.n != f->n) return FG_PACKET_INCOMPATIBLE;
...
struct FGFieldQueue {
  FGFieldSlot slots[FG_FIELD_QUEUE_SLOTS];
  uint64_t instance_id;
  unsigned n;
  atomic_uint_fast64_t published_ticket;
  uint64_t epoch, last_sequence, last_frame, clock;
  double last_time; /* consumer only */
  FGFieldQueueStats stats; /* disjoint members, read jointly only at quiescence */
};
size_t fg_field_queue_size(void) { return sizeof(FGFieldQueue); }
FGFieldQueue *fg_field_queue_init(void *memory, size_t bytes,
                                  uint64_t instance_id, uint64_t epoch, unsigned n) {
  if (!memory || bytes < sizeof(FGFieldQueue) || !instance_id || !epoch ||
      !fg_packet_size(n)) return NULL;
  FGFieldQueue *q = memory;
  memset(q, 0, sizeof(*q));
  atomic_init(&q->published_ticket, 0);
  if (!atomic_is_lock_free(&q->published_ticket)) return NULL;
  for (unsigned i = 0; i < FG_FIELD_QUEUE_SLOTS; ++i) {
...
FGPacketResult fg_field_queue_push(FGFieldQueue *q, const void *packet, size_t bytes) {
  if (!q) return FG_PACKET_MALFORMED;
  if (!packet || bytes < FG_PACKET_HEADER || bytes > FG_PACKET_MAX_BYTES) {
    ++q->stats.bad_size; return FG_PACKET_MALFORMED;
  }
  ...
  if (ticket == UINT64_MAX) return FG_PACKET_ORDER;
  for (unsigned i = 0; i < FG_FIELD_QUEUE_SLOTS; ++i) {
    FGFieldSlot *s = &q->slots[i];
  ...
  ++q->stats.full; return FG_PACKET_FULL;
}
static void release_slot(FGFieldSlot *s) {
  s->checked = 0;
  atomic_store_explicit(&s->ready, 0, memory_order_release);
}
FGPacketResult fg_field_queue_epoch(FGFieldQueue *q, uint64_t epoch) {
  if (!q || epoch <= q->epoch) return FG_PACKET_EPOCH;
  ...
  for (unsigned i = 0; i < FG_FIELD_QUEUE_SLOTS; ++i) q->slots[i].checked = 0;
  return FG_PACKET_OK;
}
FGPacketResult fg_field_queue_consume(FGFieldQueue *q, uint64_t audio_frame,
                                      FGField *f, FGPacketInfo *info) {
  if (!q || !field_storage(f) || !info) return FG_PACKET_MALFORMED;
  if (q->n != f->n) return FG_PACKET_INCOMPATIBLE;
  if (audio_frame < q->clock) {
    ++q->stats.rejected[FG_PACKET_ORDER]; return FG_PACKET_ORDER;
  }
  q->clock = audio_frame;
  FGFieldSlot *ready[FG_FIELD_QUEUE_SLOTS];
  unsigned count = 0;
  ...
  FGFieldSlot *chosen = NULL;
  for (unsigned i = 0; i < count; ++i) {
    FGFieldSlot *s = ready[i];
    if (!s->checked) {
      FGPacketResult result = fg_packet_inspect(s->packet, s->bytes, &s->info);
      if (result == FG_PACKET_OK) {
        if (s->info.instance_id != q->instance_id) result = FG_PACKET_INSTANCE;
        else if (s->info.epoch != q->epoch) result = FG_PACKET_EPOCH;
        else if (s->info.n != q->n) result = FG_PACKET_INCOMPATIBLE;
        else if (s->info.sequence <= q->last_sequence ||
                 s->info.audio_frame < q->last_frame ||
                 s->info.simulation_time < q->last_time) result = FG_PACKET_ORDER;
      }
      if (result != FG_PACKET_OK) {
  ...
  if (!chosen) return FG_PACKET_EMPTY;
...
FGFieldQueueStats fg_field_queue_stats(const FGFieldQueue *q) { return q->stats; }
