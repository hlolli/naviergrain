...
static unsigned target(const NGField *f, double x, double y, double attraction,
...
void ng_particle_observe(const NGField *f, NGParticle *p) {
  NGFieldSample s = ng_field_sample(f, p->x, p->y);
  p->omega = s.omega;
  p->strain = s.strain;
...
static unsigned recover(NGParticle *p) {
...
unsigned ng_particle_advance(const NGField *f, NGParticle *p, double dt,
                             double inertia_ms, double attraction) {
  unsigned count = recover(p);
  if (!isfinite(dt) || dt <= 0) {
    ng_particle_observe(f, p);
