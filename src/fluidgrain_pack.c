...
size_t fg_pack_resources_size(const FGGrainResources *r) {
  if(!r || !r->reader || !r->source || !r->source_length || r->source_length>INT32_MAX) return 0;
  size_t words=82+r->source_length;
  for(unsigned b=0;b<FG_SINC_BANDS;++b) words+=2*r->reader->radius[b]*(FG_SINC_PHASES+1u);
  return words<=UINT32_MAX/4 ? words*4 : 0;
}
int fg_pack_resources(const FGGrainResources *r,void *memory,size_t bytes) {
  size_t size=fg_pack_resources_size(r);
  if(!size || !memory || bytes<size) return 0;
  unsigned char *p=memory;memset(p,0,82*4);
  pack_u32(p,0x52474746u);pack_u32(p+4,r->reader->cosine?2:1);pack_u32(p+8,(uint32_t)r->source_length);
  ...
  for(unsigned b=0;b<FG_SINC_BANDS;++b) {
    ...
    unsigned n=2*r->reader->radius[b]*(FG_SINC_PHASES+1u);
    for(unsigned i=0;i<n;++i)pack_f32(p+4*word++,(float)r->reader->coefficients[b][i]);
  }
  return 1;
}
static size_t pack_header(uint32_t frames) {
  return frames<=32 ? FG_PACK_HEADER : (64u+4u*frames+63u)&~(size_t)63u;
}
size_t fg_pack_size_frames(uint32_t capacity, uint32_t frames) {
  return capacity && capacity<=4096 && frames && frames<=FG_GRAIN_PLAN_MAX_FRAMES
    ? pack_header(frames)+(size_t)capacity*frames*FG_PACK_SEGMENT : 0;
}
size_t fg_pack_scratch_size_frames(uint32_t capacity, uint32_t frames) {
  return capacity && capacity<=4096 && frames && frames<=FG_GRAIN_PLAN_MAX_FRAMES
    ? (size_t)capacity*frames*sizeof(FGPlannedGrain *) : 0;
}
size_t fg_pack_size(uint32_t capacity) { return fg_pack_size_frames(capacity,32); }
size_t fg_pack_scratch_size(uint32_t capacity) { return fg_pack_scratch_size_frames(capacity,32); }
static double pack_value(const FGPlannedGrain *g, unsigned column) {
...
static int pack_fit(const FGPlannedGrain *const *g, unsigned n, const float *times, float *coeff, int32_t *base) {
...
size_t fg_pack(const FGGrainPlan *plan,void *packet,size_t bytes,void *scratch,size_t scratch_bytes) {
  FGGrainPlanView v;
  if(!fg_grain_plan_view(plan,&v)||!packet||!scratch||bytes<fg_pack_size_frames(v.capacity,v.frames)||
     scratch_bytes<fg_pack_scratch_size_frames(v.capacity,v.frames)) return 0;
  const FGPlannedGrain **lookup=scratch;
  memset(lookup,0,fg_pack_scratch_size_frames(v.capacity,v.frames));
  ...
  for(unsigned f=0;f<v.frames;++f) {
    ...
    for(unsigned i=0;i<v.frame[f].count;++i) {
      const FGPlannedGrain *g=&v.grain[v.frame[f].offset+i];
  ...
  for(unsigned slot=0;slot<v.capacity;++slot) {
    const FGPlannedGrain **row=lookup+slot*v.frames;
    for(unsigned first=0;first<v.frames;) {
      ...
      unsigned char *segment=out+header+(size_t)segments*FG_PACK_SEGMENT;
  ...
  size_t used=header+(size_t)segments*FG_PACK_SEGMENT;
  pack_u32(out,FG_PACK_MAGIC);pack_u32(out+4,v.frames<=32?1:2);
...
int fg_pack_render(const FGGrainPlan *plan,const void *packet,size_t bytes,double *output,size_t samples) {
  FGGrainPlanView v;FGGrainResources r;
  if(!fg_grain_plan_view(plan,&v)||!fg_grain_plan_resources(plan,&r)||!packet||!output||
     bytes<FG_PACK_HEADER||samples<v.frames*2u) return 0;
  const unsigned char *p=packet;
  uint32_t segments=unpack_u32(p+12);
  size_t header=pack_header(v.frames);
  if(unpack_u32(p)!=FG_PACK_MAGIC||unpack_u32(p+4)!=(v.frames<=32?1u:2u)||
     (v.frames>32 && unpack_u32(p+40)!=header)||unpack_u32(p+8)!=v.frames||
     unpack_u32(p+16)!=v.capacity||segments>v.capacity*v.frames||
     bytes!=header+(size_t)segments*FG_PACK_SEGMENT||unpack_u32(p+20)!=bytes||
  ...
  for(unsigned j=0;j<segments;++j) {
    const unsigned char *s=p+header+j*FG_PACK_SEGMENT;
  ...
  for(unsigned j=0;j<segments;++j) {
    ...
    for(unsigned i=0;i<n;++i) {
      ...
      double value=fg_read_bandlimited(r.reader,r.source,r.source_length,phase,inc,r.loop,r.edge_samples)*env;
