...
tasks.port1.onmessage=()=>{
  ...
  try {
    if(!wasm.fg_live_render(live,512))throw new Error('The audio engine stopped. Restart playback.');
    const audio=Float32Array.from(new Float64Array(wasm.memory.buffer,wasm.fg_live_audio(live),1024),v=>Math.max(-1,Math.min(1,v)));
...
onmessage=async event=>{
  ...
  try {
    if(m.type==='init') {
      const response=await fetch('fluidgrain-live.wasm');if(!response.ok)throw new Error('The WASM engine is missing. Build the browser runtime first.');
      ...
      live=wasm.fg_live_create(m.sampleRate,-1);if(!live)throw new Error('Could not prepare the audio engine.');
      for(let i=0;i<m.controls.length;++i)if(!wasm.fg_live_control(live,i,m.controls[i]))throw new Error('Invalid initial control.');
      ...
      if(!wasm.fg_live_control(live,m.index,m.value))throw new Error('Control is out of range.');
    }else if(m.type==='view'&&live) {
      const pointer=wasm.fg_live_particles(live),header=new Float64Array(wasm.memory.buffer,pointer,10);
      const particles=Float64Array.from(new Float64Array(wasm.memory.buffer,pointer,10+header[2]*16));
      const stats=Float64Array.from(new Float64Array(wasm.memory.buffer,wasm.fg_live_stats(live),20));
      const spectrum=Float64Array.from(new Float64Array(wasm.memory.buffer,wasm.fg_live_spectrum(live),96));
      const audio=new Float64Array(wasm.memory.buffer,wasm.fg_live_audio(live),1024);
