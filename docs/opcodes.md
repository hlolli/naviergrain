...
`$FG_CONFIG_DEFAULTS` and `$FG_CONTROL_DEFAULTS`. Use the named indices to
...
[schema](../schema/fluidgrain-v1.json) lists their order, defaults and bounds.
...
are defined in `src/fluidgrain_live.h` and `ui/live-controls.js`.
...
opcode is CUDA-only and intended for offline use. Older `fluidgrain*` names
remain aliases.
